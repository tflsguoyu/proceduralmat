...
class Flake(Material):
    def __init__(self, datadir, args, isHDR, device):
        ...
        self.generateFlakes(datadir, 0.004)
        self.initFlakeNormal()       

    def generateFlakes(self, datadir, _r):
        seed = np.random.randint(10000)
        fn_out = '%sout/pdsample_%.4f_%d.out' % (datadir, _r, seed)
        cmd = "%sdata/PDSample/PDSample -r %f -s %d -o %s" % (datadir, _r, seed, fn_out)
        print(cmd)
        os.system(cmd)
    ...
    def eval_render(self, X=None):
        ...
        if self.save_tex == 'yes':
            self.albedo_tex = dstack(th.ones_like(self.geom_planar), albedo)
            self.normal_flake_tex = normal_flake
            self.rough_flake_tex = th.ones_like(self.geom_planar) * rough_flake
            self.f0_flake_tex = dstack(th.ones_like(self.geom_planar), f0_flake)
            self.rough_top_tex = th.ones_like(self.geom_planar) * rough_top
