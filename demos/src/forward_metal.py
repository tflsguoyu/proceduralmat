...
class Metal(Material):
    def __init__(self, args, isHDR, device):
        ...
        # ###synthetic
        self.paraPr = th.tensor([[999, 300, 100,2000], # light
                                 [0.4, 0.2, 0,   1], # f0_r
                                 [0.4, 0.2, 0,   1], # f0_g
                                 [0.4, 0.2, 0,   1], # f0_b
                                 [0.1, 0.1, 0,  0.8], # rough_x
                                 [0.4, 0.1, 0,  0.8], # rough_y
                                 [0.05, 0.05, 0, 0.2], # fsigma_x
                                 [ 10,  2, 0,    20], # fsigma_y
    ...
    def eval_render(self, x=None):        
        ...
        if self.save_tex == 'yes':
            ...
            self.roughx_tex = th.ones_like(n_dot_h_bump) * roughx
            self.roughy_tex = th.ones_like(n_dot_h_bump) * roughy            
            self.f0_tex = dstack(th.ones_like(n_dot_h_bump), f0)
